...
def casparStart(xmldata):
	...
	recvmsg = caspar.recv(1024).decode('utf-8')
	# recvmsg = recvmsg.split()
	print('Loading new tweet')
	print(recvmsg)
	sleep(2)
	if 'FAILED' not in recvmsg:
		cgCommand = ('CG 1-999 PLAY 1\r\n')
		cgCommand = cgCommand.encode('utf-8')
		caspar.send(cgCommand)
		print('Playing out tweet')
		print(caspar.recv(1024).decode('utf-8'))
	else:
		print('FAILED TO ADD CG, TRYING NEXT')

def casparStop():
	print('Stopping current tweet.')
...
while True:
	public_tweets = api.search(subj, lang='en')
